...
@section('content')
@php
    // Mapa de campos por pestaña, usado solo para abrir automáticamente la pestaña
    // que contiene el primer error de validación tras un intento de guardado fallido.
    $tabFieldMap = [
        'general'  => ['name', 'slug', 'category', 'tagline', 'description'],
        'contact'  => ['contact_email', 'contact_phone', 'whatsapp_phone', 'city', 'facebook_url', 'instagram_url', 'tiktok_url'],
        'payments' => ['enabled_countries', 'checkout_mode', 'payment_gateway', 'gateway_public_key', 'gateway_private_key', 'mp_access_token', 'mp_public_key'],
        'shipping' => ['national_shipping_cost', 'country_shipping_costs', 'is_express_shipping_enabled', 'express_shipping_cost'],
        'advanced' => ['build_mode', 'custom_domain', 'is_multilanguage_enabled', 'hero_title', 'hero_subtitle', 'hero_badge', 'distributors'],
    ];
    $initialTab = 'general';
    foreach ($tabFieldMap as $tabKey => $fields) {
        foreach ($fields as $f) {
            if ($errors->has($f) || $errors->has($f . '.*') || $errors->has($f . '*')) { $initialTab = $tabKey; break 2; }
        }
    }
    $supported = \App\Helpers\CurrencyHelper::supportedCountries();
@endphp
<div class="w-full max-w-5xl mx-auto space-y-6" x-data="{
    ...
    {{-- Errores de validación: siempre visibles arriba, sin importar la pestaña activa --}}
    @if($errors->any())
        <div class="glass-card p-5 border border-red-400/20 bg-red-500/5">
            <p class="text-red-400 font-bold text-sm mb-2 flex items-center gap-2">⚠️ Revisa estos campos antes de guardar:</p>
    ...
    <div class="glass-card p-2 flex flex-wrap gap-1" role="tablist" aria-label="Secciones de configuración de tienda">
        @foreach([
            ['general', '🏪', 'Datos Generales'],
            ['contact', '📱', 'Contacto y Redes'],
            ['payments', '💳', 'Moneda y Pagos'],
            ['shipping', '🚚', 'Envíos'],
            ['advanced', '⚙️', 'Avanzado'],
        ] as [$key, $icon, $label])
            <button type="button" role="tab" :aria-selected="activeTab === '{{ $key }}'" @click="activeTab = '{{ $key }}'"
                    :class="activeTab === '{{ $key }}' ? 'bg-tribio-purple/10 text-tribio-cyan border-tribio-cyan/40' : 'text-white/50 border-transparent hover:bg-white/5 hover:text-white/80'"
                    class="flex-1 min-w-[46%] sm:min-w-0 flex items-center justify-center gap-1.5 px-3 py-3 rounded-xl border text-xs sm:text-sm font-bold transition-colors">
                <span>{{ $icon }}</span><span>{{ $label }}</span>
            </button>
        @endforeach
    </div>

    <form method="POST" action="{{ route('dashboard.store.update') }}">@csrf
        ...
        <div x-show="activeTab === 'general'" x-cloak class="space-y-6">
            <div class="glass-card p-8 space-y-5">
                <div>
                    ...
                    <p class="text-xs text-white/50 mb-4">Los detalles principales de tu negocio, los que verán tus clientes primero.</p>
                </div>
            ...
            {{-- Logo y portada: forms independientes (suben archivo aparte), visibles solo en esta pestaña --}}
        ...
        <div x-show="activeTab === 'payments'" x-cloak class="space-y-6">
            <div class="glass-card p-6">
                ...
                <p class="text-xs text-white/50 mb-5">Selecciona en qué países deseas vender. El sistema autocalculará los precios con el tipo de cambio y mostrará la moneda de cada país. Las tarifas de envío por país se configuran en la pestaña <strong class="text-white/70">Envíos</strong>.</p>
        ...
        <div x-show="activeTab === 'shipping'" x-cloak class="space-y-6">
            <div class="glass-card p-6">
                ...
                {{-- Tarifas Internacionales para Países Activos (definidos en la pestaña Moneda y Pagos) --}}
                <div>
                    <label class="text-xs font-bold text-white block mb-1">Costos de Envío Internacional (por país habilitado):</label>
                    <p class="text-[11px] text-white/40 mb-3">Solo se muestran los países que activaste en la pestaña <strong class="text-white/60">Moneda y Pagos</strong>.</p>
                    <div class="space-y-3">
                        @foreach($supported as $code => $country)
                            @if($code !== 'PE')
            ...
            <div class="glass-card p-6">
                ...
                <div class="space-y-4">
                    ...
                    <div x-show="expressEnabled" x-cloak class="bg-white/3 border border-white/5 p-4 rounded-xl mt-3">
        ...
        {{-- Barra de guardado: fija abajo, visible en cualquier pestaña --}}
        <div class="sticky bottom-0 z-10 glass-card px-5 py-4 mt-6 flex flex-col sm:flex-row items-center justify-between gap-3">
            <p class="text-[11px] text-white/50 text-center sm:text-left">🔒 Los cambios de las 5 pestañas se guardan todos juntos al pulsar este botón.</p>
            <button type="submit" class="btn-primary btn-primary-lg w-full sm:w-auto justify-center">💾 Guardar Todos los Cambios</button>
        </div>
    </form>
</div>
@endsection
