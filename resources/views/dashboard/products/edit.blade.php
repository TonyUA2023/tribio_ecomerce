...
@php
    ...
    foreach ($currenciesMap as $cur => $meta) {
        $savedPrice = old('currency_prices.' . $cur, $product->currency_prices[$cur] ?? null);
        if (!empty($savedPrice) && is_numeric($savedPrice) && (float)$savedPrice > 0) { $hasManualPricing = true; break; }
    }
    $initialOptions = [];
    if (!empty($product->variant_options) && is_array($product->variant_options)) {
        foreach ($product->variant_options as $opt) {
            $vals = $opt['values'] ?? [];
            $initialOptions[] = ['name' => $opt['name'] ?? '', 'valuesText' => is_array($vals) ? implode(', ', $vals) : '', 'values' => is_array($vals) ? $vals : []];
        }
    }
    ...
    $initialVariants = $product->variants->map(fn($v) => [
        ...
        'sku' => $v->sku ?? '', 'price' => $v->price !== null ? (float)$v->price : '',
        'stock' => (int)$v->stock, 'is_active' => (bool)$v->is_active,
    ])->values();
@endphp
<div class="w-full max-w-6xl mx-auto space-y-5 sm:space-y-6">
    <form id="product-form" method="POST" action="{{ route('dashboard.productos.update', $product) }}" enctype="multipart/form-data" class="space-y-5 sm:space-y-6" x-data="{ productName: '{{ old('name', $product->name) }}' }">
        @csrf
        @method('PUT')
        ...
        <div class="sticky top-0 z-20 -mx-1 sm:mx-0">
            <div class="glass-card px-3.5 sm:px-5 py-2.5 sm:py-3 flex items-center gap-3">
                ...
                <div class="min-w-0 flex-1">
                    <p class="text-[10px] text-white/40 uppercase tracking-wider leading-none mb-0.5">Editando producto</p>
        ...
        <div id="product-images" class="glass-card p-5 sm:p-6" tabindex="-1">
            ...
            <p class="text-xs text-white/50 mb-5">Una buena foto principal es lo primero que ven tus clientes.</p>
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-5 lg:gap-6">
                <x-image-picker name="image" label="Imagen principal" :current="$product->image_path ? $product->image_url : null" save-label="Guardar cambios" />
                <div x-data="{ toRemove: [] }">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="grid grid-cols-3 sm:grid-cols-4 gap-2.5">
                            ...
                            <div class="relative group rounded-xl overflow-hidden aspect-square border border-white/10 bg-white/5" :class="{'opacity-30 border-red-400': toRemove.includes('{{ $img }}')}">
                                ...
                                <label class="absolute inset-0 flex items-center justify-center bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer">
                                    <input type="checkbox" name="remove_gallery[]" value="{{ $img }}" @change="if($el.checked) { toRemove.push('{{ $img }}') } else { toRemove = toRemove.filter(i => i !== '{{ $img }}') }" class="sr-only">
                                    <span class="text-xs font-bold px-2 py-1 rounded bg-red-600 text-white shadow" x-text="toRemove.includes('{{ $img }}') ? '✓ Deshacer' : '🗑️ Eliminar'"></span>
                                </label>
                                <template x-if="toRemove.includes('{{ $img }}')"><div class="absolute top-1 right-1 bg-red-600 text-white text-[10px] font-bold px-1.5 py-0.5 rounded">Eliminar</div></template>
                            </div>
                            @endforeach
                        </div>
                    </div>
                    @endif
                    <div x-cloak x-show="toRemove.length" class="mb-4 p-3 rounded-xl bg-white/5 border border-white/10"><p class="text-xs text-white/60">Las fotos marcadas se quitarán al guardar. Pulsa Deshacer sobre una foto para conservarla.</p></div>
                    <x-image-picker name="gallery[]" label="Fotos secundarias (galería)" :multiple="true" save-label="Guardar cambios" />
        ...
        <div class="glass-card p-5 sm:p-6" x-data="{
            videoOpen: {{ $product->video_path ? 'true' : 'false' }},
            hasVideo: {{ $product->video_path ? 'true' : 'false' }}, removeVideo: false,
            videoPreview: null, videoError: null, videoSizeText: null,
            showOnHome: {{ $product->show_video_on_home ? 'true' : 'false' }}, replaceId: '',
            ...
            handleVideoSelect(e) {
                ...
                if (file) {
                    ...
                    if (file.size > 4 * 1024 * 1024) {
                        ...
                        e.target.value = ''; this.videoPreview = null; this.videoSizeText = null;
                        return;
                    }
                    this.videoSizeText = `${sizeMB} MB`; this.videoPreview = URL.createObjectURL(file); this.removeVideo = false; this.hasVideo = true;
                } else { this.videoPreview = null; this.videoSizeText = null; }
            },
            clearVideoSelection(inputRef) {
                this.videoError = null; this.videoPreview = null; this.videoSizeText = null;
            ...
            <div x-show="videoOpen" x-cloak class="mt-5 pt-4 border-t border-white/10 space-y-4">
                    ...
                    <div class="space-y-2">
                        <div class="flex items-center justify-between flex-wrap gap-1.5">
                            <p class="input-label mb-0 text-xs text-tribio-cyan font-bold flex items-center gap-1.5">
                                <span class="w-2 h-2 rounded-full bg-tribio-cyan animate-pulse"></span><span>Nuevo video seleccionado:</span>
                            </p>
                ...
                <div x-show="!videoPreview && !removeVideo" class="space-y-2">
                    ...
                    <div class="relative rounded-2xl overflow-hidden bg-black border border-white/10 aspect-[4/5] max-w-[180px] group">
                        ...
                        <button type="button" @click="removeVideo = true; hasVideo = false; showOnHome = false;" class="absolute top-2 right-2 px-2.5 py-1 rounded-lg bg-red-600/90 hover:bg-red-600 text-white text-xs font-bold shadow-md transition">🗑️ Eliminar</button>
                    </div>
                </div>
                @endif

                <template x-if="removeVideo">
                    <div class="p-3 bg-red-500/10 border border-red-500/20 rounded-xl text-xs text-red-300 flex items-center justify-between gap-2">
                        <span>Se eliminará el video actual al guardar.</span>
                        <button type="button" @click="removeVideo = false; hasVideo = true;" class="underline font-bold text-white hover:text-red-200 flex-shrink-0">Deshacer</button>
                    </div>
                </template>
                ...
                <div class="pt-4 border-t border-white/10 space-y-3" x-show="hasVideo || videoPreview">
                    ...
                    <div x-show="showOnHome && isHomeSlotsFull && !{{ $product->show_video_on_home ? 'true' : 'false' }}" x-cloak class="p-4 rounded-xl bg-white/5 border border-white/10 space-y-3">
                        ...
                        <div class="space-y-2">
                            <template x-for="slot in currentHomeSlots" :key="slot.id">
                                <label class="flex items-center justify-between p-2.5 rounded-xl border transition cursor-pointer select-none" :class="replaceId == slot.id ? 'bg-tribio-cyan/10 border-tribio-cyan/40' : 'bg-white/3 border-white/10 hover:border-white/20'">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-5 sm:gap-6">
            ...
            <div class="lg:col-span-2 space-y-5 sm:space-y-6">
                ...
                <div class="glass-card p-5 sm:p-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea name="description" class="input-field" rows="5" placeholder="Describe los detalles de tu producto...">{{ old('description', $product->description) }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                {{-- Precio --}}
                <div class="glass-card p-5 sm:p-6" x-data="{
                    penPrice: '{{ old('price', $product->price) }}',
                    comparePenPrice: '{{ old('compare_price', $product->compare_price) }}',
                    rates: {{ json_encode($exchangeRates) }},
                    currencies: {
                        @foreach($currenciesMap as $cur => $meta)
                            @php
                                $savedPrice = old('currency_prices.' . $cur, $product->currency_prices[$cur] ?? ($cur === 'USD' && $product->price_usd > 0 ? $product->price_usd : ''));
                                $savedCompare = old('compare_currency_prices.' . $cur, $product->compare_currency_prices[$cur] ?? ($cur === 'USD' && $product->compare_price_usd > 0 ? $product->compare_price_usd : ''));
                                $isManual = !empty($savedPrice) && is_numeric($savedPrice) && (float)$savedPrice > 0;
                            @endphp
                            '{{ $cur }}': { manual: {{ $isManual ? 'true' : 'false' }}, price: '{{ $savedPrice }}', compare: '{{ $savedCompare }}' },
                        @endforeach
                    },
                    ...
                    <input type="hidden" name="price_usd" id="field_price_usd" value="{{ old('price_usd', $product->price_usd) }}">
                    <input type="hidden" name="compare_price_usd" id="field_compare_price_usd" value="{{ old('compare_price_usd', $product->compare_price_usd) }}">
                ...
                <div class="glass-card p-5 sm:p-6">
                    ...
                    <label class="flex items-center gap-3 mb-4 cursor-pointer">
                        <input type="checkbox" name="track_stock" value="1" class="w-4 h-4 accent-tribio-cyan" {{ old('track_stock', $product->track_stock) ? 'checked' : '' }}>
                ...
                <div class="glass-card p-5 sm:p-6" x-data="{
                    ...
                    generateVariants() {
                        ...
                        combinations.forEach((combo) => {
                            ...
                            newVariants.push({
                                id: existing && existing.id ? existing.id : null, title: title, attributes: attributes,
            ...
            <div class="lg:col-span-1 space-y-5 sm:space-y-6">
                ...
                <div class="glass-card p-5 sm:p-6">
                    ...
                    <div class="space-y-3">
                        <label class="flex items-center gap-3 cursor-pointer">
                            <input type="checkbox" name="is_active" value="1" class="w-4 h-4 accent-tribio-cyan" {{ old('is_active', $product->is_active) ? 'checked' : '' }}>
                            <span class="text-white/70 text-sm select-none">Activo (visible en la tienda)</span>
                        </label>
                        <label class="flex items-center gap-3 cursor-pointer">
                            <input type="checkbox" name="is_featured" value="1" class="w-4 h-4 accent-tribio-cyan" {{ old('is_featured', $product->is_featured) ? 'checked' : '' }}>
                ...
                <div class="glass-card p-5 sm:p-6" x-data="{
                    search: '',
                    selected: @js(old('categories', $product->categories->isNotEmpty() ? $product->categories->pluck('id')->toArray() : ($product->category_id ? [(int)$product->category_id] : []))),
                    primary: @js(old('category_id', $product->category_id)),
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <select name="brand_id" class="input-field">
                                ...
                                <option value="{{ $brand->id }}" {{ old('brand_id', $product->brand_id) == $brand->id ? 'selected' : '' }}>🏷️ {{ $brand->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
