...
<div class="max-w-3xl" x-data="{
    ...
    <form method="POST" action="{{ route('dashboard.categorias.store') }}" enctype="multipart/form-data" class="space-y-6">
        @csrf
        ...
        <div class="glass-card p-6 space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="name" class="input-field" value="{{ old('name') }}" required placeholder="Ej: Ropa, Cocina, Calzado">
                </div>
                <div>
            ...
            <div>
                ...
                <div class="flex items-center gap-3">
                    <input type="color" name="color" value="{{ old('color', '#8B5CF6') }}" class="h-10 w-20 rounded-xl border-0 bg-transparent cursor-pointer">
        ...
        <div class="glass-card p-6 space-y-3">
            ...
            <label class="flex items-start gap-3 cursor-pointer p-2.5 rounded-xl hover:bg-white/5 transition">
                <input type="checkbox" name="is_featured" value="1" {{ old('is_featured', true) ? 'checked' : '' }} class="mt-0.5 accent-tribio-purple w-4 h-4 rounded">
                <div>
            ...
            <label class="flex items-start gap-3 cursor-pointer p-2.5 rounded-xl hover:bg-white/5 transition">
                <input type="checkbox" name="show_in_header" value="1" {{ old('show_in_header') ? 'checked' : '' }} class="mt-0.5 accent-tribio-cyan w-4 h-4 rounded">
                <div>
                    <span class="text-white font-bold text-sm flex items-center gap-1.5">
            ...
            <label class="flex items-start gap-3 cursor-pointer p-2.5 rounded-xl hover:bg-white/5 transition">
                <input type="checkbox" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} class="mt-0.5 accent-tribio-purple w-4 h-4 rounded">
                <div>
        ...
        <div class="flex gap-3 pt-2">
            <button type="submit" class="btn-primary btn-primary-lg">Guardar Categoría</button>
