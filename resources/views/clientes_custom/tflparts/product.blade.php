        ...
        <div class="bg-white rounded-3xl border border-gray-200 overflow-hidden shadow-sm p-6 sm:p-10 animate-fade-in-up">
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-12">
                ...
                <div class="lg:col-span-6 space-y-6">
                    <div class="grid grid-cols-2 gap-4 border-b border-gray-100 pb-4 text-xs font-semibold uppercase tracking-wider text-gray-500">
                        <div>
                            <p class="text-[9px] text-gray-400 font-bold mb-0.5">Código SKU</p>
                            <p class="text-gray-900 font-black text-sm">{{ $product->sku ?? 'N/D' }}</p>
                        </div>
                        <div>
                            <p class="text-[9px] text-gray-400 font-bold mb-0.5">Categoría</p>
                            <p class="text-gray-900 font-black text-sm">{{ $product->category->name ?? 'Repuestos' }}</p>
                        </div>
                    </div>

                    <div class="space-y-1">
                        @if($product->category)
                            <span class="text-[10px] font-black text-[#E50914] uppercase tracking-widest block">{{ $product->category->name }}</span>
                        @endif
                    ...
                    <div class="p-6 bg-gray-50/80 rounded-2xl border border-gray-200/60 space-y-4">
                        <div class="flex items-baseline justify-between">
                            <div class="flex items-baseline gap-3">
                                <span class="text-3xl font-black text-gray-900">S/. {{ number_format($product->price, 2) }}</span>
                            ...
                            @if($product->compare_price && $product->compare_price > $product->price)
                                ...
                                @endphp
                                <span class="px-2.5 py-1 bg-red-100 text-[#E50914] text-[10px] font-black uppercase rounded shadow-sm">
                                    Ahorra {{ $discount }}%
                                </span>
                            @endif
                        </div>
                        <div class="flex flex-col gap-2 pt-2.5 border-t border-gray-200/60 text-[10px] text-gray-500 font-bold uppercase tracking-wider">
                            <div class="flex items-center gap-2">
                                <svg class="w-4 h-4 text-green-600 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/>
                                </svg>
                                <span>Garantía de Fábrica Certificada</span>
                            </div>
                            <div class="flex items-center gap-2">
                                <svg class="w-4 h-4 text-green-600 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <span>Envío garantizado a nivel nacional</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col sm:flex-row gap-4">
                        <button @click="addToCart({{ $product->id }}, '{{ addslashes($product->name) }}', {{ $product->price }}, '{{ $product->image_path ? $product->image_url : '' }}')"
                                class="px-6 py-4 font-black uppercase tracking-wider text-xs rounded-xl bg-black hover:bg-gray-800 text-white text-center flex-1 shadow-lg hover:-translate-y-0.5 transition-all flex items-center justify-center gap-2">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"/>
                            </svg>
                            Añadir al Carrito
                        ...
                        @if($store->whatsapp_phone)
                            <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $store->whatsapp_phone) }}?text={{ urlencode('Hola, estoy interesado en el repuesto: ' . $product->name . ' (SKU: ' . $product->sku . '). ¿Tienen disponibilidad?') }}"
                               target="_blank"
                               class="px-6 py-4 font-black uppercase tracking-wider text-xs rounded-xl bg-[#16A34A] hover:bg-green-700 text-white text-center flex-1 shadow-lg hover:-translate-y-0.5 transition-all flex items-center justify-center gap-2">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
