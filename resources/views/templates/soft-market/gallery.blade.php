...
<div class="bg-white" x-data="{ mobileMenuOpen: false, searchOpen: false }">
    ...
    <!-- Page Header -->
    <div class="bg-[var(--t-bg)] py-10 md:py-16 text-center border-b border-gray-200">
        <h1 class="text-4xl md:text-5xl font-bold text-[#1A1A1A] mb-4 font-brand">Inspiración</h1>
            ...
            <div class="columns-1 md:columns-2 lg:columns-3 gap-6 space-y-6">
                ...
                <div class="break-inside-avoid relative group rounded-2xl overflow-hidden shadow-sm bg-gray-100 cursor-pointer">
                    ...
                    <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                        @if($item->link_url)
                            <a href="{{ $item->link_url }}" target="_blank" class="px-6 py-2 bg-white text-[#1A1A1A] font-bold rounded-full hover:bg-[var(--t-primary)] hover:text-[var(--t-on-primary)] transition-colors transform translate-y-4 group-hover:translate-y-0 duration-300">
                                Ver Detalle
                            </a>
                        @endif
                    </div>
