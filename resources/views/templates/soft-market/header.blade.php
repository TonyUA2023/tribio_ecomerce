...
<!-- Top Announcement Bar (owner-editable) -->
@if(($templatePreview ?? false) || $storefrontTheme->enabled('announcement.enabled'))
<div class="bg-[var(--t-primary-100)] border-b border-[var(--t-primary-200)] py-2 px-3 sm:px-6 text-center text-[11px] sm:text-xs font-semibold text-[var(--t-primary-dark)] tracking-wide flex items-center justify-center gap-2 font-brand"
     data-tpl-show="announcement.enabled" @unless($storefrontTheme->enabled('announcement.enabled')) hidden @endunless>
    <span class="inline-block w-1.5 h-1.5 rounded-full bg-[var(--t-primary)] animate-pulse"></span>
    <span data-tpl-text="announcement.text">{{ $storefrontTheme->text('announcement.text') }}</span>
</div>
@endif

<!-- Header Component -->
<header class="bg-[var(--t-bg)] sticky top-0 z-50 border-b border-stone-200/70 shadow-xs backdrop-blur-md" x-data="{ mobileMenuOpen: false, searchOpen: false }">
    <div class="max-w-screen-2xl mx-auto px-3 sm:px-6 lg:px-12 py-3 sm:py-4 md:py-4.5">
        ...
        <div class="flex justify-between items-center">
            ...
            <div class="flex-1 flex items-center gap-1 sm:gap-2.5 min-w-0">
                ...
                <div class="relative shrink-0" x-data="{ 
                    ...
                    <button @click="langOpen = !langOpen" @click.away="langOpen = false" 
                            class="flex items-center gap-1 px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-full bg-white hover:bg-stone-50 text-[#1E1D1B] hover:text-[var(--t-primary)] text-[11px] sm:text-xs font-bold tracking-wider transition border border-stone-200 shadow-2xs whitespace-nowrap cursor-pointer">
                    ...
                    <div x-show="langOpen" style="display: none;" 
                        ...
                        <button type="button" @click="switchLanguage('ES')" 
                                ...
                                :class="currentLang === 'ES' ? 'text-[var(--t-primary)] font-bold bg-[var(--t-bg)]' : 'text-[#1E1D1B]'">
                            <span>Español</span>
                            <span x-show="currentLang === 'ES'" class="text-[var(--t-primary)] text-xs font-bold">✓</span>
                        </button>
                        <button type="button" @click="switchLanguage('EN')" 
                                class="w-full text-left px-4 py-2 text-xs font-semibold hover:bg-stone-50 flex items-center justify-between" 
                                :class="currentLang === 'EN' ? 'text-[var(--t-primary)] font-bold bg-[var(--t-bg)]' : 'text-[#1E1D1B]'">
                            <span>English</span>
                            <span x-show="currentLang === 'EN'" class="text-[var(--t-primary)] text-xs font-bold">✓</span>
                        </button>
                    </div>
                </div>
                @endif
                ...
                <div class="relative shrink-0" x-data="{ 
                    ...
                    <button @click="currOpen = !currOpen" @click.away="currOpen = false" 
                            class="flex items-center gap-1 sm:gap-1.5 px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-full bg-white hover:bg-stone-50 text-[#1E1D1B] hover:text-[var(--t-primary)] text-[11px] sm:text-xs font-bold tracking-wider transition border border-stone-200 shadow-2xs cursor-pointer whitespace-nowrap flex-nowrap shrink-0"
                    ...
                    <div x-show="currOpen" style="display: none;" 
                        ...
                        @foreach($headerCountries as $hCode => $hData)
                            <button type="button" @click="switchCountry('{{ $hCode }}', '{{ $hData['currency'] }}')" 
                                    ...
                                    :class="currentCountry === '{{ $hCode }}' ? 'text-[var(--t-primary)] font-bold bg-[var(--t-bg)]' : 'text-[#1E1D1B]'">
                                ...
                                <div class="flex items-center gap-1.5">
                                    ...
                                    <span x-show="currentCountry === '{{ $hCode }}'" class="text-[var(--t-primary)] text-xs font-bold">✓</span>
                                </div>
                            </button>
                        @endforeach
                    </div>
                ...
                <a href="{{ route('store.show', $store->slug) }}" class="pointer-events-auto flex items-center gap-2 group">
                    ...
                    @else
                        <div class="flex flex-col items-center relative py-1">
                            <span class="font-bold text-2xl sm:text-3xl md:text-4xl tracking-tight text-[#1E1D1B] group-hover:text-[var(--t-primary)] transition-colors font-brand leading-none">
                                {{ $store->name }}
                            </span>
                            <!-- Brand smile arc under the store name -->
                            <svg class="w-16 sm:w-20 md:w-24 h-2 mt-0.5 text-[var(--t-primary)]" viewBox="0 0 100 12" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div class="flex-1 flex items-center justify-end space-x-1.5 sm:space-x-3 md:space-x-4">
                ...
                <button @click="$dispatch('open-customer-modal')" 
                        class="text-[#1E1D1B] hover:text-[var(--t-primary)] p-2 rounded-full hover:bg-stone-100/70 transition hidden md:flex items-center gap-1.5 cursor-pointer" 
                ...
                <button @click="searchOpen = true" 
                        class="text-[#1E1D1B] hover:text-[var(--t-primary)] p-2 rounded-full hover:bg-stone-100/70 transition hidden md:flex items-center justify-center cursor-pointer" 
                ...
                <button onclick="window.dispatchEvent(new CustomEvent('open-cart-drawer'))"
                        class="text-[#1E1D1B] hover:text-[var(--t-primary)] p-2 rounded-full hover:bg-stone-100/70 transition relative cursor-pointer" 
                    ...
                    <span data-cart-count class="absolute -top-0.5 -right-0.5 bg-[var(--t-primary)] text-[var(--t-on-primary)] text-[9px] font-bold px-1.5 py-0.5 rounded-full min-w-[18px] text-center shadow-xs font-brand">0</span>
                </button>
        ...
        <nav class="hidden md:flex flex-wrap justify-center items-center gap-x-7 lg:gap-x-9 gap-y-2 mt-4 pb-1 font-brand">
            <a href="{{ route('store.show', $store->slug) }}" 
               class="text-base lg:text-[17px] font-semibold transition-colors duration-200 {{ request()->routeIs('store.show') ? 'text-[var(--t-primary)] font-bold border-b-2 border-[var(--t-primary)] pb-1' : 'text-[#363430] hover:text-[var(--t-primary)]' }}">
                {{ $isEn ? 'Home' : 'Inicio' }}
            </a>
            
            <a href="{{ route('store.catalog', $store->slug) }}" 
               class="text-base lg:text-[17px] font-semibold transition-colors duration-200 {{ (request()->routeIs('store.catalog') && !request()->has('category')) ? 'text-[var(--t-primary)] font-bold border-b-2 border-[var(--t-primary)] pb-1' : 'text-[#363430] hover:text-[var(--t-primary)]' }}">
                {{ $isEn ? 'Shop' : 'Catálogo' }}
            </a>
            ...
            @foreach($headerCategories as $cat)
                ...
                <a href="{{ route('store.catalog', ['slug' => $store->slug, 'category' => $cat->slug]) }}" 
                   class="text-base lg:text-[17px] font-semibold transition-colors duration-200 {{ $isCatActive ? 'text-[var(--t-primary)] font-bold border-b-2 border-[var(--t-primary)] pb-1' : 'text-[#363430] hover:text-[var(--t-primary)]' }}">
                    {{ $cat->getTranslatedName() }}
                </a>
            @endforeach
            
            <a href="{{ route('store.contact', $store->slug) }}" 
               class="text-base lg:text-[17px] font-semibold transition-colors duration-200 {{ request()->routeIs('store.contact') ? 'text-[var(--t-primary)] font-bold border-b-2 border-[var(--t-primary)] pb-1' : 'text-[#363430] hover:text-[var(--t-primary)]' }}">
    ...
    <div x-show="mobileMenuOpen" style="display: none;" class="md:hidden fixed inset-0 z-[60] flex justify-end">
        ...
        <div x-show="mobileMenuOpen"
             ...
             class="relative h-full w-[86%] max-w-sm bg-[var(--t-bg)] border-l border-stone-200 shadow-2xl flex flex-col font-brand">
            ...
            <div class="flex-1 overflow-y-auto px-5 py-5 space-y-6">
                ...
                <form action="{{ route('store.catalog', $store->slug) }}" method="GET" class="relative">
                    <input type="text" name="search"
                           ...
                           class="w-full pl-10 pr-4 py-2.5 bg-white border border-stone-200 rounded-full text-sm text-stone-800 placeholder-stone-400 focus:outline-none focus:border-[var(--t-primary)] focus:ring-2 focus:ring-[var(--t-primary)]/20 transition shadow-2xs">
                    <button type="submit" class="absolute left-3.5 top-1/2 -translate-y-1/2 text-stone-400 hover:text-[var(--t-primary)] transition cursor-pointer" title="Buscar">
                ...
                <button @click="$dispatch('open-customer-modal'); mobileMenuOpen = false"
                        class="w-full flex items-center gap-3 px-4 py-3.5 rounded-2xl bg-gradient-to-r from-[var(--t-primary-50)] to-[var(--t-secondary-50)] border border-[var(--t-primary-200)] hover:shadow-md active:scale-[0.99] transition text-left cursor-pointer">
                    <span class="flex items-center justify-center w-10 h-10 rounded-full bg-white text-[var(--t-primary)] shrink-0 shadow-2xs">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
                    </span>
                    <span class="flex-1 min-w-0">
                        <span class="block text-sm font-bold text-[#1E1D1B] leading-tight">{{ $isEn ? 'My Account' : 'Mi Cuenta' }}</span>
                        <span class="block text-xs text-[var(--t-primary-dark)] leading-tight mt-0.5">{{ $isEn ? 'Tribio orders & profile' : 'Pedidos Tribio y perfil' }}</span>
                    </span>
                    <svg class="w-4 h-4 text-[var(--t-primary)] shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
                </button>

                <!-- Primary Navigation -->
                <nav class="space-y-1.5">
                    <a href="{{ route('store.show', $store->slug) }}"
                       class="flex items-center gap-3 px-3 py-2.5 rounded-xl transition {{ request()->routeIs('store.show') ? 'bg-white shadow-2xs text-[var(--t-primary)]' : 'text-stone-700 hover:bg-white/70' }}">
                        <span class="flex items-center justify-center w-8 h-8 rounded-full bg-[var(--t-primary-50)] text-[var(--t-primary-dark)] shrink-0">
                    ...
                    <a href="{{ route('store.catalog', $store->slug) }}"
                       class="flex items-center gap-3 px-3 py-2.5 rounded-xl transition {{ (request()->routeIs('store.catalog') && !request()->has('category')) ? 'bg-white shadow-2xs text-[var(--t-primary)]' : 'text-stone-700 hover:bg-white/70' }}">
                        <span class="flex items-center justify-center w-8 h-8 rounded-full bg-[var(--t-primary-50)] text-[var(--t-primary-dark)] shrink-0">
                    ...
                    <a href="{{ route('store.contact', $store->slug) }}"
                       class="flex items-center gap-3 px-3 py-2.5 rounded-xl transition {{ request()->routeIs('store.contact') ? 'bg-white shadow-2xs text-[var(--t-primary)]' : 'text-stone-700 hover:bg-white/70' }}">
                        <span class="flex items-center justify-center w-8 h-8 rounded-full bg-[var(--t-secondary-50)] text-[var(--t-secondary-dark)] shrink-0">
                ...
                @if($headerCategories->isNotEmpty())
                    <div>
                        ...
                        <div class="flex flex-wrap gap-2">
                            @foreach($headerCategories as $cat)
                                <a href="{{ route('store.catalog', ['slug' => $store->slug, 'category' => $cat->slug]) }}"
                                   class="px-3.5 py-1.5 rounded-full text-[13px] font-semibold bg-white border border-stone-200 text-stone-700 hover:border-[var(--t-primary)] hover:text-[var(--t-primary)] transition">
                                    {{ $cat->getTranslatedName() }}
                                </a>
                            @endforeach
                        </div>
                ...
                <!-- Trust Badge -->
                <div class="flex items-center justify-center gap-2 text-[11px] font-semibold text-[var(--t-primary-dark)] bg-[var(--t-primary-100)] rounded-xl py-2.5">
                    <span class="inline-block w-1.5 h-1.5 rounded-full bg-[var(--t-primary)]"></span>
