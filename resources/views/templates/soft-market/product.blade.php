            ...
            <div class="space-y-4 lg:sticky lg:top-8" x-data="{ activeMedia: 'image', activeImage: @js($product->image_url), isMuted: true }">
                ...
                <div class="flex items-center gap-3 overflow-x-auto pb-2 scrollbar-thin">
                    ...
                    <button type="button" @click="activeMedia = 'video'" 
                            ...
                            :class="activeMedia === 'video' ? 'border-[var(--t-primary)] ring-2 ring-[var(--t-primary)]/30 scale-105' : 'border-stone-300 opacity-75 hover:opacity-100'">
                        <span class="text-xl">🎬</span>
                        <span class="text-[9px] font-black uppercase tracking-wider mt-0.5 text-[var(--t-primary)]">Video</span>
                    </button>
                    @endif
            ...
            <div class="flex flex-col" x-data="{
                ...
                <div class="flex items-center gap-2 mb-3 flex-wrap">
                    ...
                    @if($product->isMadeToOrder($store))
                        <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[11px] font-bold bg-[var(--t-primary-50)] text-[var(--t-primary-deep)] border border-[var(--t-primary-200)]">
                            ✂️ Hecho a pedido{{ $product->lead_time_days ? ' · listo en ' . $product->lead_time_days . ' ' . ($product->lead_time_days === 1 ? 'día' : 'días') : '' }}
                        </span>
                ...
                <div class="mb-6 p-4 rounded-2xl bg-[#F4F1EA]/60 border border-stone-200/60">
                    ...
                    <a href="#detalles-producto" class="inline-flex items-center gap-1 text-xs font-bold text-[var(--t-primary)] hover:text-[#1A1A1A] mt-2 transition">
                    ...
                    <template x-for="opt in options" :key="opt.name">
                        <div>
                            <div class="flex items-center justify-between text-xs font-bold mb-2">
                                ...
                                <span class="text-[var(--t-primary)] font-mono" x-text="selectedAttributes[opt.name] || ''"></span>
                            </div>
                ...
                @if($product->isMadeToOrder($store))
                    ...
                    @include('templates.soft-market._made-to-order')
                @else
                ...
                <div class="space-y-3 mb-8">
                    ...
                    <button type="button" 
                            @click="buyNow()" 
                            :disabled="isOutOfStock"
                            class="w-full bg-[var(--t-primary)] hover:bg-[var(--t-primary-dark)] text-[var(--t-on-primary)] font-extrabold text-sm sm:text-base rounded-2xl transition-all shadow-md py-3.5 px-6 flex items-center justify-center gap-2 cursor-pointer active:scale-98"
...
{{ $product->description }}
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-12 md:mt-20">
        @include('components.storefront.reviews', ['variant' => 'soft'])
    </div>

    <!-- Related Products ("Productos Relacionados") -->
    @if($relatedProducts->count() > 0)
    <div class="bg-[var(--t-bg)] py-20 border-t border-stone-200/60 mt-16">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center max-w-xl mx-auto mb-12">
                <span class="text-xs font-bold uppercase tracking-widest text-[var(--t-primary)] mb-2 block">{{ \App\Helpers\TranslationHelper::isEn() ? 'Recommended for you' : 'Recomendados para ti' }}</span>
                ...
                <div class="group bg-white rounded-3xl p-3 border border-stone-200/80 shadow-xs hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between cursor-pointer"
                    ...
                    <div>
                        ...
                        <div class="px-1">
                            ...
                            @endif
                            <h3 class="text-[#1A1A1A] font-bold text-sm line-clamp-2 mb-2 group-hover:text-[var(--t-primary)] transition-colors leading-snug">{{ $related->name }}</h3>
                        </div>
                    </div>
