@extends('templates.soft-market.layout')
...
@endphp
<div class="bg-[var(--t-bg)] min-h-screen text-[#1A1A1A]" x-data="{ mobileMenuOpen: false, searchOpen: false }">
                ...
                <form id="catalogFilterForm" action="{{ route('store.catalog', $store->slug) }}" method="GET" class="space-y-6 sticky top-32">
                    ...
                    <div>
                        ...
                        <div class="space-y-1">
                            <button type="button" onclick="setCategoryFilter('')" 
                                    class="w-full flex justify-between items-center text-sm py-1.5 px-2.5 rounded-lg transition text-left {{ !request('category') ? 'text-[var(--t-primary)] font-bold bg-[var(--t-primary)]/10' : 'text-gray-600 hover:text-[#1A1A1A] hover:bg-stone-100/60' }}">
                            ...
                            @foreach($categories as $cat)
                                ...
                                <button type="button" onclick="setCategoryFilter('{{ $isSelected ? '' : $cat->slug }}')"
                                        class="w-full flex justify-between items-center text-sm py-1.5 px-2.5 rounded-lg transition text-left {{ $isSelected ? 'text-[var(--t-primary)] font-bold bg-[var(--t-primary)]/10' : 'text-gray-600 hover:text-[#1A1A1A] hover:bg-stone-100/60' }}">
                                    <span class="truncate pr-2">{{ $cat->getTranslatedName() }}</span>
                                    <span class="text-xs px-2 py-0.5 rounded-full flex-shrink-0 transition {{ $isSelected ? 'bg-[var(--t-primary)] text-[var(--t-on-primary)] font-bold' : 'bg-gray-100 text-gray-500' }}">
                    ...
                    <div class="pt-4 border-t border-gray-200/60">
                        ...
                        <div class="space-y-1 max-h-52 overflow-y-auto pr-1">
                            <button type="button" onclick="setBrandFilter('')" 
                                    class="w-full flex justify-between items-center text-sm py-1.5 px-2.5 rounded-lg transition text-left {{ !request('brand') ? 'text-[var(--t-primary)] font-bold bg-[var(--t-primary)]/10' : 'text-gray-600 hover:text-[#1A1A1A] hover:bg-stone-100/60' }}">
                            ...
                            @foreach($brands as $b)
                                ...
                                <button type="button" onclick="setBrandFilter('{{ $isBrandSelected ? '' : $b->slug }}')"
                                        class="w-full flex justify-between items-center text-sm py-1.5 px-2.5 rounded-lg transition text-left {{ $isBrandSelected ? 'text-[var(--t-primary)] font-bold bg-[var(--t-primary)]/10' : 'text-gray-600 hover:text-[#1A1A1A] hover:bg-stone-100/60' }}">
                                    <span class="truncate pr-2">{{ $b->name }}</span>
                                    <span class="text-xs px-2 py-0.5 rounded-full flex-shrink-0 transition {{ $isBrandSelected ? 'bg-[var(--t-primary)] text-[var(--t-on-primary)] font-bold' : 'bg-gray-100 text-gray-500' }}">
                                        {{ $b->products_count }}
                                    </span>
                                </button>
                            @endforeach
                    ...
                    <div class="pt-4 border-t border-gray-200/60">
                        ...
                        <div class="flex items-center gap-2 mb-2.5">
                            <div class="relative flex-1">
                                ...
                                <input type="number" step="any" name="min_price" id="minPriceInput" value="{{ request('min_price') }}" placeholder="Min" class="w-full pl-7 pr-2 py-2 text-xs bg-white border border-gray-300 rounded-lg focus:ring-1 focus:ring-[var(--t-primary)] focus:border-[var(--t-primary)] outline-none">
                            </div>
                            <span class="text-gray-400 font-bold">-</span>
                            <div class="relative flex-1">
                                <span class="absolute left-2.5 top-1/2 -translate-y-1/2 text-xs font-bold text-gray-400">{{ $currencySymbol }}</span>
                                <input type="number" step="any" name="max_price" id="maxPriceInput" value="{{ request('max_price') }}" placeholder="Max" class="w-full pl-7 pr-2 py-2 text-xs bg-white border border-gray-300 rounded-lg focus:ring-1 focus:ring-[var(--t-primary)] focus:border-[var(--t-primary)] outline-none">
                            </div>
                        </div>
                        {{-- Presets rápidos de precio --}}
                        <div class="grid grid-cols-3 gap-1.5">
                            <button type="button" onclick="setQuickPrice('', 50)" class="px-2 py-1 text-[11px] font-medium border border-gray-200 rounded-lg bg-white hover:border-[var(--t-primary)] hover:text-[var(--t-primary)] transition text-center {{ request('max_price') == 50 && !request('min_price') ? 'border-[var(--t-primary)] text-[var(--t-primary)] font-bold bg-[var(--t-primary)]/10' : '' }}">&lt; 50</button>
                            <button type="button" onclick="setQuickPrice(50, 100)" class="px-2 py-1 text-[11px] font-medium border border-gray-200 rounded-lg bg-white hover:border-[var(--t-primary)] hover:text-[var(--t-primary)] transition text-center {{ request('min_price') == 50 && request('max_price') == 100 ? 'border-[var(--t-primary)] text-[var(--t-primary)] font-bold bg-[var(--t-primary)]/10' : '' }}">50 - 100</button>
                            <button type="button" onclick="setQuickPrice(100, '')" class="px-2 py-1 text-[11px] font-medium border border-gray-200 rounded-lg bg-white hover:border-[var(--t-primary)] hover:text-[var(--t-primary)] transition text-center {{ request('min_price') == 100 && !request('max_price') ? 'border-[var(--t-primary)] text-[var(--t-primary)] font-bold bg-[var(--t-primary)]/10' : '' }}">&gt; 100</button>
                        </div>
                    </div>
                    ...
                    <div class="pt-4 border-t border-gray-200/60 space-y-2">
                        <label class="flex items-center gap-2 text-xs font-medium text-gray-700 cursor-pointer hover:text-black">
                            <input type="checkbox" name="on_sale" value="1" {{ request('on_sale') ? 'checked' : '' }} onchange="this.form.submit()" class="rounded border-gray-300 text-[#1A1A1A] focus:ring-[var(--t-primary)]">
                            <span>{{ \App\Helpers\TranslationHelper::isEn() ? '🔥 On sale / Discount' : '🔥 En oferta / descuento' }}</span>
                        </label>
                        <label class="flex items-center gap-2 text-xs font-medium text-gray-700 cursor-pointer hover:text-black">
                            <input type="checkbox" name="in_stock" value="1" {{ request('in_stock') ? 'checked' : '' }} onchange="this.form.submit()" class="rounded border-gray-300 text-[#1A1A1A] focus:ring-[var(--t-primary)]">
                    ...
                    <div class="pt-4 border-t border-gray-200/60">
                        ...
                        <select name="sort" class="w-full text-xs py-2.5 px-3 bg-white border border-gray-300 rounded-lg focus:ring-1 focus:ring-[var(--t-primary)] focus:border-[var(--t-primary)] outline-none" onchange="this.form.submit()">
                    ...
                    <div class="space-y-2 pt-2">
                        <button type="submit" class="w-full py-3 bg-[#1A1A1A] hover:bg-[var(--t-primary)] hover:text-[var(--t-on-primary)] text-white font-bold text-sm rounded-lg transition-colors shadow-sm">
            ...
            <div class="flex-1">
                ...
                @if($activeFiltersCount > 0)
                    <div class="mb-6 flex flex-wrap items-center gap-2 p-3 bg-white rounded-xl border border-stone-200/80 shadow-sm">
                        ...
                        @if(request('category'))
                            ...
                            @endphp
                            <a href="{{ request()->fullUrlWithQuery(['category' => null]) }}" class="inline-flex items-center gap-1.5 px-3 py-1 bg-[var(--t-primary)]/15 text-[var(--t-primary)] rounded-full text-xs font-bold transition hover:bg-[var(--t-primary)] hover:text-[var(--t-on-primary)]/25">
                        ...
                        @if(request('brand'))
                            ...
                            @endphp
                            <a href="{{ request()->fullUrlWithQuery(['brand' => null]) }}" class="inline-flex items-center gap-1.5 px-3 py-1 bg-[var(--t-primary)]/15 text-[var(--t-primary)] rounded-full text-xs font-bold transition hover:bg-[var(--t-primary)] hover:text-[var(--t-on-primary)]/25">
                ...
                @if($products->isEmpty())
                    <div class="text-center py-20 bg-gray-50 rounded-2xl border border-dashed border-gray-300">
                        ...
                        <a href="{{ route('store.catalog', $store->slug) }}" class="inline-block bg-[#1A1A1A] text-white px-6 py-2 rounded-full font-medium hover:bg-[var(--t-primary)] hover:text-[var(--t-on-primary)] transition">{{ \App\Helpers\TranslationHelper::isEn() ? 'Clear Filters' : 'Limpiar Filtros' }}</a>
                    </div>
                @else
