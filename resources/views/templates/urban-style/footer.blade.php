...
<footer class="bg-[#1E1D1B] text-stone-300 font-sans border-t border-stone-800 selection:bg-[var(--t-primary)] selection:text-white"
        x-data="{
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
            <div class="flex flex-col lg:flex-row items-center justify-between gap-6">
                <div class="text-center lg:text-left max-w-xl">
                    <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[var(--t-primary)]/20 text-[var(--t-primary-200)] text-xs font-bold uppercase tracking-wider mb-2 border border-[var(--t-primary)]/30 font-brand">
                    ...
                    @else
                        <h3 class="text-xl sm:text-2xl font-bold text-white tracking-tight font-brand" data-tpl-text="footer.newsletter_title">{{ $storefrontTheme->text('footer.newsletter_title') }}</h3>
                        <p class="text-xs sm:text-sm text-stone-400 mt-1">
                        ...
                        <form @submit.prevent="submitNewsletter()" class="flex flex-col sm:flex-row gap-2">
                            <div class="relative flex-1">
                                ...
                                <input type="email" x-model="newsletterEmail" required
                                       ...
                                       class="w-full bg-white/10 border border-white/20 text-white placeholder-stone-400 text-xs sm:text-sm rounded-xl py-3 pl-10 pr-4 outline-none focus:border-[var(--t-primary)] focus:ring-2 focus:ring-[var(--t-primary)]/30 transition">
                            </div>
                            <button type="submit"
                                    class="px-6 py-3 bg-[var(--t-primary)] hover:bg-[var(--t-primary-dark)] text-[var(--t-on-primary)] font-bold text-xs sm:text-sm rounded-xl transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-1.5 whitespace-nowrap cursor-pointer font-brand">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-10">
            ...
            <div class="lg:col-span-2 space-y-4">
                <div class="flex items-center gap-3">
                    ...
                    @else
                        <div class="flex flex-col">
                            ...
                            <svg class="w-16 h-2 mt-0.5 text-[var(--t-primary)]" viewBox="0 0 100 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <p class="text-xs sm:text-sm text-stone-400 leading-relaxed pr-4" data-tpl-text="footer.about">{{ $storefrontTheme->isCustomized('footer.about') || $store->description ? $storefrontTheme->text('footer.about') : ($store->tagline ?: $storefrontTheme->text('footer.about')) }}</p>

                {{-- Puntos de Contacto Directo --}}
                <div class="space-y-2 pt-2 text-xs">

                    {{-- Email (solo si la tienda lo configuró) --}}
                    @if($store->contact_email)
                    <a href="mailto:{{ $store->contact_email }}"
                       class="flex items-center gap-2.5 text-gray-300 hover:text-[var(--t-primary)] transition">
                ...
                {{-- Redes Sociales (solo las que la tienda configuró) --}}
                @if($store->instagram_url || $store->tiktok_url || $store->facebook_url)
                <div class="pt-2">
                    ...
                    <div class="flex items-center gap-2.5">
                        ...
                        {{-- TikTok --}}
                        @if($store->tiktok_url)
                        <a href="{{ $store->tiktok_url }}" target="_blank" rel="noopener" 
            ...
            <div>
                <h4 class="text-xs font-bold uppercase tracking-wider text-white mb-4 border-l-2 border-[var(--t-primary)] pl-2.5">
                    {{ $isEn ? 'Collections' : 'Catálogo' }}
                </h4>
                <ul class="space-y-2.5 text-xs text-gray-400">
                    <li>
                        <a href="{{ route('store.catalog', $store->slug) }}" class="hover:text-[var(--t-primary)] transition flex items-center gap-1.5 font-semibold text-gray-200">
                    ...
                    @if(method_exists($store, 'galleryItems') && $store->galleryItems()->exists())
                        <li class="pt-1 border-t border-white/5">
                            <a href="{{ route('store.gallery', $store->slug) }}" class="hover:text-[var(--t-primary)] transition flex items-center gap-1.5">
                ...
                <ul class="space-y-2.5 text-xs text-gray-400">
                    <li>
                        <button type="button" @click="window.openCustomerModal ? window.openCustomerModal('orders') : null"
                                class="hover:text-[var(--t-primary)] transition flex items-center gap-1.5 text-left cursor-pointer font-medium text-gray-300">
            ...
            <div>
                ...
                <div class="mb-5">
                    <button type="button" @click="reclamacionesOpen = true" 
                            class="w-full p-2.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 hover:border-[var(--t-primary)]/40 transition text-left cursor-pointer group">
                        <div class="flex items-center gap-2.5">
                            <div>
                                <span class="text-xs font-bold text-white block group-hover:text-[var(--t-primary)] transition">Libro de Reclamaciones</span>
            ...
            <div class="flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-gray-500">
                <div>
                    ...
                    <p class="text-[11px] text-gray-600 mt-0.5">
                        ...
                        <a href="https://tribio.com" target="_blank" rel="noopener" class="text-gray-400 hover:text-[var(--t-primary)] font-bold transition">Tribio eCommerce</a>
                    </p>
                </div>
    ...
    <div x-show="reclamacionesOpen" style="display: none;"
        ...
        <div @click.away="reclamacionesOpen = false"
                ...
                <form @submit.prevent="submitReclamo()" class="space-y-4 text-xs">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                            ...
                            <input type="text" x-model="reclamo.nombres" required class="w-full bg-white/5 border border-white/10 rounded-lg p-2.5 text-white outline-none focus:border-[var(--t-primary)]">
                        </div>
                        <div>
                            <label class="block font-bold text-gray-300 mb-1">Documento (DNI / CE / Pasaporte) *</label>
                            <input type="text" x-model="reclamo.documento_num" required class="w-full bg-white/5 border border-white/10 rounded-lg p-2.5 text-white outline-none focus:border-[var(--t-primary)]" placeholder="Ej: 74581234">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                            ...
                            <input type="email" x-model="reclamo.email" required class="w-full bg-white/5 border border-white/10 rounded-lg p-2.5 text-white outline-none focus:border-[var(--t-primary)]" placeholder="tu@correo.com">
                        </div>
                        <div>
                            <label class="block font-bold text-gray-300 mb-1">Teléfono / WhatsApp *</label>
                            <input type="text" x-model="reclamo.telefono" required class="w-full bg-white/5 border border-white/10 rounded-lg p-2.5 text-white outline-none focus:border-[var(--t-primary)]" placeholder="999 999 999">
                        </div>
                    </div>

                    <div>
                        <label class="block font-bold text-gray-300 mb-1">Dirección / Ciudad</label>
                        <input type="text" x-model="reclamo.direccion" class="w-full bg-white/5 border border-white/10 rounded-lg p-2.5 text-white outline-none focus:border-[var(--t-primary)]">
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2 border-t border-white/5">
                        <div>
                            ...
                            <div class="flex gap-4 pt-1">
                                <label class="flex items-center gap-1.5 cursor-pointer">
                                    <input type="radio" value="Reclamo" x-model="reclamo.tipo" class="text-[var(--t-primary)]">
                                    <span>Reclamo (Disconformidad con producto)</span>
                                </label>
                                <label class="flex items-center gap-1.5 cursor-pointer">
                                    <input type="radio" value="Queja" x-model="reclamo.tipo" class="text-[var(--t-primary)]">
                        ...
                        <div>
                            ...
                            <input type="text" x-model="reclamo.monto" class="w-full bg-white/5 border border-white/10 rounded-lg p-2.5 text-white outline-none focus:border-[var(--t-primary)]" placeholder="Ej: Pedido #1024 o S/ 150.00">
                        </div>
                    </div>

                    <div>
                        <label class="block font-bold text-gray-300 mb-1">Detalle del Reclamo o Queja *</label>
                        <textarea rows="3" x-model="reclamo.detalle" required class="w-full bg-white/5 border border-white/10 rounded-lg p-2.5 text-white outline-none focus:border-[var(--t-primary)]" placeholder="Describe claramente lo ocurrido..."></textarea>
                    </div>

                    <div>
                        <label class="block font-bold text-gray-300 mb-1">Pedido del Consumidor</label>
                        <textarea rows="2" x-model="reclamo.pedido" class="w-full bg-white/5 border border-white/10 rounded-lg p-2.5 text-white outline-none focus:border-[var(--t-primary)]" placeholder="Qué solución solicitas (ej: cambio de producto, reembolso, etc.)..."></textarea>
                    </div>
                    ...
                    <div class="pt-3 flex justify-end gap-3">
                        ...
                        <button type="submit" class="px-6 py-2.5 rounded-xl bg-[var(--t-primary)] hover:bg-[var(--t-primary-dark)] text-[var(--t-on-primary)] font-bold transition shadow-lg cursor-pointer">
            ...
            <div x-show="reclamo.enviado" style="display: none;" class="text-center py-8">
                ...
                <div class="inline-block px-4 py-2 rounded-xl bg-white/5 border border-[var(--t-primary)]/40 font-mono text-[var(--t-primary)] font-bold text-base mb-4">
                ...
                <button type="button" @click="reclamacionesOpen = false; resetReclamo();" class="px-6 py-2.5 rounded-xl bg-[var(--t-primary)] text-[var(--t-on-primary)] font-bold text-xs cursor-pointer">
                    Entendido / Cerrar
                </button>
            </div>
        </div>
    ...
    <div x-show="faqOpen" style="display: none;"
        ...
        <div @click.away="faqOpen = false"
            ...
            <div class="space-y-3 text-xs" x-data="{ activeAccordion: 1 }">
                ...
                <div class="p-3.5 rounded-xl bg-white/5 border border-white/10">
                    <button type="button" @click="activeAccordion = activeAccordion === 1 ? null : 1" class="w-full flex items-center justify-between font-bold text-left text-white cursor-pointer">
                        ...
                        <span x-text="activeAccordion === 1 ? '−' : '+'" class="text-base text-[var(--t-primary)]"></span>
                    </button>
                ...
                <div class="p-3.5 rounded-xl bg-white/5 border border-white/10">
                    <button type="button" @click="activeAccordion = activeAccordion === 2 ? null : 2" class="w-full flex items-center justify-between font-bold text-left text-white cursor-pointer">
                        ...
                        <span x-text="activeAccordion === 2 ? '−' : '+'" class="text-base text-[var(--t-primary)]"></span>
                    </button>
                ...
                <div class="p-3.5 rounded-xl bg-white/5 border border-white/10">
                    <button type="button" @click="activeAccordion = activeAccordion === 3 ? null : 3" class="w-full flex items-center justify-between font-bold text-left text-white cursor-pointer">
                        ...
                        <span x-text="activeAccordion === 3 ? '−' : '+'" class="text-base text-[var(--t-primary)]"></span>
                    </button>
                ...
                <div class="p-3.5 rounded-xl bg-white/5 border border-white/10">
                    <button type="button" @click="activeAccordion = activeAccordion === 4 ? null : 4" class="w-full flex items-center justify-between font-bold text-left text-white cursor-pointer">
                        ...
                        <span x-text="activeAccordion === 4 ? '−' : '+'" class="text-base text-[var(--t-primary)]"></span>
                    </button>
            ...
            <div class="mt-6 pt-4 border-t border-white/10 text-center">
                ...
                @if($store->contact_email)
                <a href="mailto:{{ $store->contact_email }}" class="inline-flex items-center justify-center px-6 py-2.5 rounded-xl bg-white/10 hover:bg-white/20 border border-white/10 text-white font-bold text-sm transition">
                    Escríbenos a: {{ $store->contact_email }}
                </a>
                @else
                <a href="{{ route('store.contact', $store->slug) }}" class="inline-flex items-center justify-center px-6 py-2.5 rounded-xl bg-white/10 hover:bg-white/20 border border-white/10 text-white font-bold text-sm transition">
    ...
    <div x-show="shippingPolicyOpen" style="display: none;"
        ...
        <div @click.away="shippingPolicyOpen = false"
            ...
            <div class="mt-6 pt-4 border-t border-white/10 text-right">
                <button type="button" @click="shippingPolicyOpen = false" class="px-5 py-2 rounded-xl bg-[var(--t-primary)] text-[var(--t-on-primary)] font-bold text-xs cursor-pointer">
                    Entendido
                </button>
            </div>
        </div>
